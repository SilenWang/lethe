...
def _dynamic_keys() -> set[str]:
    """Keys resolved at runtime (not literal ``tr("...")`` calls): the review-table
    source badges, the engine's language names / sizes / notes, and the
    browser-store error messages the UI maps onto locale keys."""
    import app as lethe_app

    keys = {label for label, _ in lethe_app.SOURCE_BADGE.values()}
    keys |= set(lethe_app.LANG_LABEL_KEYS.values())
    keys |= set(lethe_app.ENGINE_NOTE_KEYS.values())
    keys |= {key for _prefix, key in lethe_app.STORE_ERROR_KEYS}
    # the fallback sentence handed to _store_error(...) at each call site
    keys |= set(re.findall(r'_store_error\(tr, exc, "([\w.]+)"', _app_source()))
    return keys
...
@pytest.mark.parametrize("text", [
    ...
    "Browser data (IndexedDB)", "Migrate old server-side data", "Erase all browser data",
    "Clear result files", "Export backup (.json)", "Import backup (.json)",
    "Migrate old data into this browser", "Import old data into this browser",
    "Server-side data folder (program resources only)",
    "Server-side temporary files (auto-deleted)",
    # model rows (dev's per-model Settings list)
    "In use", "Not installed", "Detect with this model from the next document on",
    "Download this result file again",
    ...
    # dev's browser-storage copy
    "Browser storage is unavailable", "Your browser data for Lethe looks cleared",
    "Saved in this browser (IndexedDB)", "The reversal key could NOT be saved in this browser",
    "Server-side working copies", "Result files: the last", "Persistent storage is ON",
    "This dictionary is kept in this browser only", "Backup imported",
    "Your entity dictionary, custom token types, conversion history",
    "Couldn't read the conversion history from this browser",
])
...
def test_every_engine_language_and_note_is_translatable():
    """Settings → Detection & OCR languages renders the engine's table: every
    language name, size annotation and prose note must map to a locale key."""
    import app as lethe_app
    ...
    for lang in nlp_suggester.LANGUAGES:
        ...
        if lang.get("note"):
            assert lang["note"] in lethe_app.ENGINE_NOTE_KEYS, \
                f"engine language note has no locale key: {lang['note']!r}"
        for model in lang["models"]:
            size = model["size"]
            assert any(ch.isdigit() for ch in size), \
                f"size annotation is prose, not a number + unit: {size!r}"
            if model.get("note"):
                assert model["note"] in lethe_app.ENGINE_NOTE_KEYS, \
                    f"engine model note has no locale key: {model['note']!r}"
    for key in set(lethe_app.ENGINE_NOTE_KEYS.values()):
        assert key in EN and key in ZH, f"{key} missing from a locale file"


def test_settings_language_rows_go_through_the_translator():
    """Regression guard: the engine's English labels and notes must never be
    rendered straight into the localised Settings page."""
    source = _ui_source()
    assert re.search(r'ui\.label\(\s*L\["label"\]', source) is None
    assert re.search(r'ui\.label\(\s*L\["note"\]', source) is None
    assert re.search(r'ui\.label\(\s*m\["note"\]', source) is None
    assert re.search(r'_lang_name\(tr,\s*L\["code"\],\s*L\["label"\]\)', source)
    assert re.search(r'_engine_note\(tr,\s*L\["note"\]\)', source)
    assert re.search(r'_engine_note\(tr,\s*m\["note"\]\)', source)


def test_engine_error_text_is_never_shown_raw():
    """The browser store (client-store.js) and the model helpers report failures
    in English; showing str(exc) on its own would leak English into the Chinese
    UI, so the known failures get their own locale strings."""
    source = _ui_source()
    assert "notify(str(exc)" not in source
    # set_active_model's own English message must not be shown verbatim
    assert 'ui.notify(msg, color="positive" if ok else' not in source
    for key in ("reid.wrong_passphrase", "reid.key_missing", "reid.result_gone"):
        assert key in EN and key in ZH, f"{key} missing from a locale file"
    assert "Couldn't reach browser storage" not in EN  # diagnostic detail, not UI copy


def test_dynamic_language_helpers_resolve_against_the_locales():
    """The Settings page resolves engine text at runtime; each helper must return
    the locale value and fall back to the engine's own wording for anything the
    locale files don't know (the ``i18n.has_key`` path)."""
    import app as lethe_app
    from lethe import nlp_suggester

    tr_zh = Translator("zh")
    assert lethe_app._lang_name(tr_zh, "en", "English") == ZH["lang.en"]
    assert lethe_app._lang_name(tr_zh, "xx", "Klingon") == "Klingon"
    note = nlp_suggester.LANGUAGES[0]["models"][0]["note"]
    assert lethe_app._engine_note(tr_zh, note) == ZH[lethe_app.ENGINE_NOTE_KEYS[note]]
    assert lethe_app._engine_note(tr_zh, "an unknown engine note") == "an unknown engine note"
    tr_en = Translator("en")
    assert lethe_app._lang_name(tr_en, "zh", "Chinese") == EN["lang.zh"]
    assert lethe_app._ttl_text(tr_en) == EN["ttl.minutes"].format(count="5")
